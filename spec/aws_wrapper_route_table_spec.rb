  ...
  module Ec2
    describe RouteTable do
      ...
      it "creates a RouteTable named \'#{RTABLE_NAME}\'" do
        rt_info = AwsWrapper::Ec2::RouteTable.create(RTABLE_NAME, {:name => VPC_NAME})
        created_rtables << rt_info[:route_table_id]
        expect(AwsWrapper::Ec2::RouteTable.exists?(:name => RTABLE_NAME)).to be true
      end

      it "deletes a RouteTable named \'#{RTABLE_NAME}\'" do
        AwsWrapper::Ec2::RouteTable.delete(:name => RTABLE_NAME)
        expect(AwsWrapper::Ec2::RouteTable.exists?(:name => RTABLE_NAME)).not_to be true
      end

      it "adds an entry to the table" do
        rt_info = AwsWrapper::Ec2::RouteTable.create(RTABLE_NAME, {:name => VPC_NAME})
        created_rtables << rt_info[:route_table_id]
        rt = AwsWrapper::Ec2::RouteTable.new(:id => rt_info[:route_table_id])
        igw = AwsWrapper::Ec2::InternetGateway.create(IGW_NAME, {:name => VPC_NAME})
      ...
      it "deletes an entry from the table" do
        rt_info = AwsWrapper::Ec2::RouteTable.find(:name => RTABLE_NAME)
        rt = AwsWrapper::Ec2::RouteTable.new(:id => rt_info[:route_table_id])
